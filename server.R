...
url_display<-eventReactive(input$CHECK, {
  url()
    ...
    data<-eventReactive(input$IMPORT, {
      ...
      progress<-shiny::Progress$new()
      progress$set(message = "Downloading Data, please be patient, this may take some time.", value = 0)
      on.exit(progress$close())
...
outputOptions(output, 'All_Data', suspendWhenHidden=TRUE)
    ...
    output$Save_data1 <- downloadHandler(
        ...
        content = function(con) {
           ...
           write.table(all_data(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
        })
    ...
    output$Save_data6 <- downloadHandler(
      ...
      content = function(con) {
          ...
          write.table(filtered_data(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
      })    
    no_units<-reactive({
    ...
    meta_gen_no_units <- reactive({
        data <- no_units()
        metadate <- data.table(x = "Date:", y = paste(Sys.time()))
        metadata <- data.table(x = "Dataset:", y = "No Units")
        metaurl <- data.table(x = "URL:", y = url_display())
        metanondet <- data.table(x = "Method for non-detects:", y = non_detect_method())
        metaorg <- data.table(x = "Number of organizations:", y = length(unique(data$Organization)))
        metastat <- data.table(x = "Number of stations:", y = length(unique(data$Station)))
    ...
    output$Save_data3 <- downloadHandler(
      ...
      content = function(con) {
          ...
          write.table(no_units(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
      })
    ...
    meta_gen_no_methods <- reactive({
      metadate <- data.table(x = "Date:", y = paste(Sys.time()))
      metadata <- data.table(x = "Dataset:", y = "No Methods")
      metaurl <- data.table(x = "URL:", y = url_display())
      metanondet <- data.table(x = "Method for non-detects:", y = non_detect_method())
      metaorg <- data.table(x = "Number of organizations:", y = length(unique(data$Organization)))
      metastat <- data.table(x = "Number of stations:", y = length(unique(data$Station)))
    ...
    output$Save_data4 <- downloadHandler(
      ...
      content = function(con) {
          ...
          write.table(no_methods(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
      })
    duplicates<-reactive({
      data.frame(data_dt()[duplicate_logic()])
    })
    ...
    meta_gen_duplicates <- reactive({
        data <- duplicates()
        metadate <- data.table(x = "Date:", y = paste(Sys.time()))
        metadata <- data.table(x = "Dataset:", y = "Duplicates")
        metaurl <- data.table(x = "URL:", y = url_display())
        metanondet <- data.table(x = "Method for non-detects:", y = non_detect_method())
        metaorg <- data.table(x = "Number of organizations:", y = length(unique(data$Organization)))
        metastat <- data.table(x = "Number of stations:", y = length(unique(data$Station)))
    ...
    output$Save_data5 <- downloadHandler(
      ...
      content = function(con) {
          ...
          write.table(duplicates(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
     })
    ...
    output$Save_data2 <- downloadHandler(
      ...
      content = function(con) {
          ...
          write.table(non_detects(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
      })
    ...
    output$Save_Summary_Data <- downloadHandler(
      ...
      content = function(con) {
          ...
          write.table(summarized(), con, row.names = F, append = TRUE, quote = TRUE, sep = ",")
      })
...
meta_gen_mapdata <- reactive({
  data <- dat_display()
  metadate <- data.table(x = "Date:", y = paste(Sys.time()))
  metadata <- data.table(x = "Dataset:", y = "Filtered Map Data")
  metaurl <- data.table(x = "URL:", y = url_display())
  metanondet <- data.table(x = "Method for non-detects:", y = non_detect_method())
  metaorg <- data.table(x = "Number of organizations:", y = length(unique(data$Organization)))
  metastat <- data.table(x = "Number of stations:", y = length(unique(data$Station)))
  ...
  output$save_map_data <- downloadHandler(
        ...
        content = function(con) {
          write.table(meta_gen_mapdata(), con, row.names = F, col.names = FALSE, sep = ",")  
          write.table(data.frame(dat_display()), con, row.names = F,append = TRUE, quote = TRUE, sep = ",")
        })
